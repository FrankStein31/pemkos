...
class TenantPresenter(private  val activity: Activity, private val view: TenantView) {
    ...
    fun getTenant(){
        try{
            subscription = apiServices.tenant()
    ...
    fun searchAbk(id: String, keyword: String){
        try{
            subscription = apiServices.searchTenant(id = id, keyword = keyword)
                    .subscribeOn(Schedulers.newThread())
                    .observeOn(AndroidSchedulers.mainThread())
                    .doOnSubscribe {
                        customProgress.showLoading()
